...
// TODO Page::with()
class AdminPagesController extends AdminModelController {
...
/**/
	...
	protected function getListData($itemId = null)
	{
		return array(
			...
			'layouts' => Layout::with('fields')->get()->toArray(),
...
protected function getItems($orderBy = 'id', $desc = false)
	...
	protected function getNestedItems($pages, $level = 0, $items = array())
	{
		foreach ($pages as $page)
		{
			$items[] = array(
			...
			$items = $this->getNestedItems($subPages, $level + 1, $items);
		}
