	...
	window.App = new Vue({
		...
		methods: {
			isItemInRouteOrPlan(itemId) {
				const isItemInPlan = this.myRoutePlanItems.find(e => e.id == itemId)
				const isItemInRoute = this.myRouteItems.find(e => e.items.find(x => x.id == itemId))
			...
			filterItems() {
				...
				if(this.searchSelectedCat) {
					...
					nested = nested.filter(e => {
						return Array.isArray(this.searchSelectedCat) ? this.searchSelectedCat.includes(e.layout_id) : e.layout_id == this.searchSelectedCat
						
					})
				}
				
				if(this.selectedCity.id) {
					console.log('F: selectedCity',this.selectedCity.id)
					nested = nested.filter(e => e.city_id == this.selectedCity.id)
		...
		mounted() {
			...
			ymaps.ready(function() {
				...
				redrawMap(_this.filteredItems)
				console.log('_this.selectedCity', _this.selectedCity)
			...
			const seoListParamsRaw = this.$el.dataset.searchParams
			const seoListParams = seoListParamsRaw.split(';').reduce((a, b) => {
				const arr = b.split(':')
				a[arr[0]] = arr[1].split(',')
				return a
			}, {})
			
			console.log(seoListParams)
			
			if(seoListParams.city) {
				let f = this.cities.find(e => seoListParams.city.includes(e.id))
				if(f) {
					this.selectedCity = f
				}
			}
			
			if(seoListParams.tags) {
				this.tags = seoListParams.tags.map(e => ({text: e}))
			}
			
			if(seoListParams.cats) {
				this.searchSelectedCat = seoListParams.cats
			}
