...
document.addEventListener('DOMContentLoaded', function() {
	...
	Vue.component('city-switcher', {
		...
		computed: {
			...
			citySwitcherTitle() {
				if(!this.selectedcity) return 'Город/место'
				return this.selectedcity.city ? this.selectedcity.city : 'Город/место'
			}
		},
		methods: {
			citySwitchClose() {
			...
			setCity(city) {
				...
				this.citySwitchClose()
			}
		},
		mounted() {
			if(jQuery('#citysw-search')) {
				try {
					this.citiesData =  JSON.parse(jQuery('#citysw-search .city-switcher__list').attr('data-json'))
				} catch(err) {
					console.log(err)
				}
			}
		...
		template: `
		<div class="city-switcher city-switcher--vsearch-recs" :class="{'city-switcher--active': isCitySwitcherShow}" v-click-outside="citySwitchClose" id="citysw" > 
			<div class="rbtn rbtn--text city-switcher__title" @click="isCitySwitcherShow = true" v-cloak>
				...
				<div class="city-switcher__list" data-json="{$ cats|json_encode() $}" v-show="isCitySwitcherShow" v-cloak>
					...
					<div class="city-switcher__items">
						<div class="city-switcher__listitem" v-for="city in citySwitcherFilteredList">
							<a href="#" @click.prevent="setCity(city)">{{ city.city }}</a>
						</div>
					</div>
				</div>
			</transition>
	...
	const dragHandler = jQuery(window).width() > 768 ? false : '.vcard-move'
	...
	window.VUE_VDNH = new Vue({
		...
		data: function() {
			return {
				...
				myRouteStartPoint: qstore.state.myRoute.startPoint || null,
				myRouteCurrentDay: 0,
		...
		watch: {
			selectedObject(v) {
				if(v) {
					this.selectedObjectFetchContent()
					this.mapSetCenter(v)
				} else {
					if(window.vdnhMap && window.myRouteSelectedObjectPlacemark) {
						window.vdnhMap.geoObjects.remove(window.myRouteSelectedObjectPlacemark)
						window.myRouteSelectedObjectPlacemark = false
						this.drawMap() //!!!
					} 
				}
			},
					...
					try {
						if(this.isPageItemsLoaded) {
							...
							v.forEach(async function(e) {
								...
								const data = await drawRoute(getRoutePoints(itemsToDraw, _this.myRouteStartPoint), _this.myRouteStartPoint, _this.mapRouteMode, true)
		...
		methods: {
			onMyRoutePlanItemsAdd(...args) {
				console.log('onMyRoutePlanItemsAdd', args)
			},
			async selectedObjectFetchContent() {
				try {
					const params = {
						'select': 'field_content'
					}
					let data = await apiService.pages.show(this.selectedObject.id, params)
					console.log(data)
					this.$set(this.selectedObject, 'field_content', data.data.field_content)
				} catch(err) {
					console.log(err)
				}
			},
			scrollTo(el) {
				const topPos = jQuery(el).offset().top - 90
				console.log({topPos})
				window.scrollTo({
				  	top: topPos,
				  	behavior: "smooth",
			...
			closeRecsOptionsDropdown() {
				this.isRecsOptionsDropdownShowed = false
			},
			setGenRouteCity(city) {
				if(city && city.city_id) {
					this.generateRouteQuizSelectedOptions.city = city
				}
			},
			generateRouteFromPlan(isCloseGQZ = false) {
				if(!this.myRouteItems.length) return alert('Выберите даты маршрута')
				
				const shuffledPlan = [...this.myRoutePlanItems].sort(() => Math.random() > 0.5 ? true : false)
				console.log({shuffledPlan})
				
				const n = this.myRouteItems.length
				
				const allObjectsLength = shuffledPlan.length + this.myRouteItems.reduce((a,b) => {
					a += b.items.length
					return a
				}, 0)
				
				const itemsPerDay = Math.floor(allObjectsLength / n)
				
				console.log({itemsPerDay})
				
				const chunks = []
				for (let i = n; i > 0; i--) {
					chunks.push(shuffledPlan.splice(0, Math.ceil(shuffledPlan.length / i)));
				}
				
				console.log({chunks})
				if(chunks.length) {
					this.myRouteItems.forEach((e, i) => {
						
					})
					chunks.forEach((e, i) => {
						e.forEach(ei => {
							qstore.addItemToMyRouteFromPlan(ei, this.myRouteItems[i].date_id)
						})
					})
				}
			...
			findInside(circle) {
				if(window.SEARCH_MAP) {
					...
					const myRouteItems = this.myRouteItems.reduce((a, b) => {
						a.push(...b.items)
						return a
					}, [])
					const combinedObjects = [...this.myRoutePlanItems, ...myRouteItems]
					//console.log({combinedObjects})
				
					const mainPoints = getRoutePoints(combinedObjects).map(e => new ymaps.Placemark(e, {
						data: {
							object_id: 'f232'
						}
					}))
					
					const allItems = window.SEARCH_ITEMS.map(e => e.item_data)
					
					const allPoints = allItems.map(e => {
						const itemCoords = getItemCoordsByField(e.field_coords)
						if(itemCoords) {
							return new ymaps.Placemark(itemCoords, {
								data: {
									object_id: e.id
								}
							})
					...
					let objects = ymaps.geoQuery(allPoints)
					//console.log(circle)
					...
					//console.log({itemsInsideRadius})
					return itemsInsideRadius
				}
				//
			},
			calcNearRecs(radius = 500) {
				const myRouteItems = this.myRouteItems.reduce((a, b) => {
					a.push(...b.items)
					return a
				}, [])
				const combinedObjects = [...this.myRoutePlanItems, ...myRouteItems]
				const combinedObjectsId = combinedObjects.map(e => e.id)
				//console.log({combinedObjects})

				const mainPoints = getRoutePoints(combinedObjects)
				
				const results = []
				
				mainPoints.forEach(e => {
					const circleCoords = {
						latitude: e[0], 
						longitude: e[1]
					}
					results.push(...setSearchGeo(circleCoords, radius))
				})
				
				//console.log({combinedObjectsId})
				const uniqueResults = Array.from(new Set(results)).filter(e => !combinedObjectsId.includes(e))
			...
			generateMyRouteRecommendations() {
				...
				const recsItems = window.SEARCH_ITEMS.filter(e => {
					return uniqueRecsIds.includes(e.item_data.id)
				}).map(e => e.item_data)
			...
			getDateText2(date) {
				return new Date(date).toLocaleString('default', { month: 'numeric', day: 'numeric' }).replaceAll('.', '/')
			},
			...
			myRouteItemImgSrc(item) {
				...
				if(item && Array.isArray(item.field_gallery) && item.field_gallery.length > 0 && typeof(item.field_gallery[0]) == 'string') return `/site/data/images/medium/${item.field_gallery[0]}`
				if(item.field_image) return `/site/data/images/medium/${item.field_image}`
				return '/site/theme/images/noimage.png'
			},
			isMyRouteItemHasImg(item) {
				//console.log(item.field_gallery)
				if(item && Array.isArray(item.field_gallery) && item.field_gallery.length > 0) return true
				if(item.field_image) return true
				return false
			},
			...
			generateFakeRoute() {
				...
				res = res.filter(e => {
					...
					return e.item_data.layout_id != '17' && e.item_data.layout_id != '51' && !isItemGenerated
				})
				
				console.log('good layouts', res)
				
				if(this.generateRouteQuizSelectedOptions.price.range) {
					res = res.filter(e => {
						return Number(e.item_data.field_prices) >= this.generateRouteQuizSelectedOptions.price.range[0] && Number(e.item_data.field_prices) <= this.generateRouteQuizSelectedOptions.price.range[1]
					})
					console.log('price filter', res)
				}
				
				if(this.generateRouteQuizSelectedOptions.city) {
					res = res.filter(e => {
						return e.item_city == this.generateRouteQuizSelectedOptions.city.city_id
					})
					console.log('city filter', res)
				}
				
				if(this.generateRouteQuizSelectedOptions.count.items[0] == 3) {
					let countTags = ['для детей', 'с детьми']
					res = res.filter(e => {
						return e.item_data.field_tags.split(',').map(t => t.trim().toLowerCase()).some(x => {
							return countTags.map(a => {
								return a.trim().toLowerCase()
							}).includes(x)
						})
					})
					console.log('kids filter', res)
				}
				
				if(this.generateRouteQuizSelectedOptions.count.items[0] == 4) {
					let countTags2 = ['с животными', 'с питомцем']
					res = res.filter(e => {
						return e.item_data.field_tags.split(',').map(t => t.trim().toLowerCase()).some(x => {
							return countTags2.map(a => {
								return a.trim().toLowerCase()
							}).includes(x)
						})
					})
					console.log('pets filter', res)
				}
				
				if(this.generateRouteQuizSelectedOptions.tags.items.length) {
					
					const combinedTags = _this.generateRouteQuizSelectedOptions.tags.items
					
					res = res.filter(e => {
						return e.item_data.field_tags.split(',').map(t => t.trim().toLowerCase()).some(x => {
							return combinedTags.map(a => {
								return a.trim().toLowerCase()
							}).includes(x)
						})
					})
					console.log('tags filter', res)
				}
			...
			setRecsSlider() {
				this.$nextTick().then(() => {
					//slider.destroy()
					if(window.myRouteRecsSlider) {
						window.myRouteRecsSlider.destroy()
					}
					window.myRouteRecsSlider = new Splide('.vsearch-recs-slider-main', {
						type: 'loop',
						perPage: 2,
						pagination: false,
					  	gap: '8px',
					...
					window.myRouteRecsSlider.refresh();	
				})
			},
			mapSetCenter(item) {
				if(window.vdnhMap) {
					if(item.field_coords && item.field_coords.split(',').map(x => Number(x.trim())).length == 2) {
						const coords = item.field_coords.split(',').map(x => Number(x.trim()))
						window.vdnhMap.setCenter(coords, 16)
						var mapHeight = document.querySelector('.search-item--map').offsetHeight
						var top = (mapHeight - 250) / 2
						var position = window.vdnhMap.getGlobalPixelCenter();
						
						if(window.myRouteSelectedObjectPlacemark) {
							window.vdnhMap.geoObjects.remove(window.myRouteSelectedObjectPlacemark)
							window.myRouteSelectedObjectPlacemark = false
						} 
						window.myRouteSelectedObjectPlacemark = new ymaps.Placemark(window.vdnhMap.getCenter(), {
							 data: [
								{weight: 10, color: '#e85768'}
							],
							iconContent: ''
						}, {
							...
							iconPieChartCoreRadius: 5,
							// Стиль заливки центральной части.
							iconPieChartCoreFillStyle: '#ffffff',
							// Cтиль линий-разделителей секторов и внешней обводки диаграммы.
							iconPieChartStrokeStyle: '#e85768',
							// Ширина линий-разделителей секторов и внешней обводки диаграммы.
							iconPieChartStrokeWidth: 3,
							// Максимальная ширина подписи метки.
							iconPieChartCaptionMaxWidth: 0
						});
						window.vdnhMap.geoObjects.add(window.myRouteSelectedObjectPlacemark)
						console.log('placemark', window.myRouteSelectedObjectPlacemark)
						
						window.vdnhMap.setGlobalPixelCenter([position[0], position[1] + top]);
						//window.vdnhMap.setGlobalPixelCenter([coords[0], coords[1] + 0.0200], 16)
					}
				}
			}
		},
		computed: {
			myRoutePlanItemsList: {
				get() {
					return []
					return this.myRoutePlanItems
				},
				set(value) {
					console.log('wefwfwefwefwfwefw', value)
					//return []
					//this.myRoutePlanItems.push(value)
		...
		async mounted() {
			...
			if (url.searchParams.has('show-generator')) {
				this.isGenerateRouteQuizShowed = true
			}
	
			const userPrefs = jQuery('#vsearch-recs').attr('data-userprefs').split(',').map(e => e.trim()).filter(Boolean)
			//console.log({userPrefs})
			this.recsOptions.prefs.userPrefs = userPrefs
			...
			const searchPoints = getItemsMapObjects(allItems)
			//console.log({searchPoints})
			await initSearchMap(searchPoints)
			this.isSearchMapLoaded = true
	...
	*/
	...
	function setSearchGeo(crd, radius) {
		...
		window.circleGeo = new ymaps.Circle([[crd.latitude, crd.longitude], radius], null, { 
			draggable: true,
			fillColor: "#14c5d0",
			fillOpacity: 0.2,
			// Цвет обводки.
		...
		window.circleGeo.events.add('dragend', function() {
			return window.VUE_VDNH.findInside(window.circleGeo)
		})
		SEARCH_MAP.geoObjects.removeAll()
		SEARCH_MAP.geoObjects.add(window.circleGeo);
		return window.VUE_VDNH.findInside(window.circleGeo)
	}

	async function initSearchMap(points = []) {
		console.log('SEARCH_MAP init')
		window.SEARCH_MAP = new ymaps.Map('map-search', {
			center: [55.728791, 37.577454],
			zoom: 12,
			controls: ['zoomControl']
		})
		
		if(points.length) {
			console.log('SEARCH_MAP points', points.length)
			...
			window.SEARCH_MAP.geoObjects.add(objectManager)
		}
	}
	...
	function getRoutePoints(items = [], start = null) {
		...
		res = [...res, ...items
			.filter(e => {
				//console.log(e.id, e.field_coords)
		...
		console.log('getRoutePoints', res)
		return res
	}

	function getItemCoordsByField(field_coords) {
		if(field_coords && field_coords.split(',').map(x => Number(x.trim())).length == 2) {
			return field_coords.split(',').map(x => Number(x.trim()))
		}
		return false
	}

	function getItemsMapObjects(items) {
		let markers = items.map((e, i) => {
			//if(e.field_coords && e.field_coords.split(',').length == 2) {
			let coords = e.field_coords ? e.field_coords.split(',').map(n => Number(n.trim())) : []
			if(coords.length != 2 || coords.some(c => isNaN(c))) {
				console.log('Bad coords: id ', e.id, e.field_coords)
				coords = [53, 53]
			}
			const itemGal = e.field_gallery ? (e.field_gallery[0] ? `<a href="${e.path}"><img style="object-fit: cover; border-radius: 4px; width: 200px; height: 140px;" src="/site/data/images/medium/${e.field_gallery[0]}"></a>` : '') : ''
			return {
				"type":"Feature",
				"id": e.created_at,
				"geometry": { 
				...
				"properties":{ 
					...
					"balloonContentBody": `<div class="baloon-body">${itemGal}
