...
import { USER, PAGE_ID, LAYOUT_ID, parseObject } from './utils.js'
...
document.addEventListener('DOMContentLoaded', function() {
	...
	const catFilters = {
		hotels: {
			...
			sreda: [
				...
				{
					name: 'Лифт для коляски',
					value: 'Лифт для коляски'
				}
			],
			food: [
				{
		...
		places: {
			types: [
				...
				{name: 'Сервис', value: 'Сервис'}
			],
			service: [
				{name: 'Wi-fi', value: 'Wi-fi'},
				{name: 'Оплата картой', value: 'Оплата картой'},
				{name: 'Коляска', value: 'Коляска'},
				{name: 'Зарядить телефон', value: 'Бери заряд'},
				{name: 'Комната матери и ребенка', value: 'Комната матери и ребенка'}
			]
		},
		excursion: {
			types: [
		...
		events: {
			...
			service: [
				{name: 'Wi-fi', value: 'Wi-fi'},
				{name: 'Оплата картой', value: 'Оплата картой'},
				{name: 'Программа лояльности RussPass', value: 'Программа лояльности'}
			]
		}
	}
	...
	let dataItems = parseObject($('.data-pages').attr('data-answer'), [])
	...
	let dataGal = dataItems.map(e => {
		...
		let regex = /\/(small|medium|large)\/(.*?)\.(jpg|png)/g;
	...
	window.App = new Vue({
		...
		data: function() {
			return {
				...
				pageNumber: 1,
				isMapLoaded: false,
				myRouteItems: qstore.state.myRoute.routeItems,
				myRoutePlanItems: qstore.state.myRoute.planItems,
				isUserPurchasesUpdated: false,
				isBuyObjectModalShowed: false,
				buyObjectModalItem: null,
				shared: qstore.state,
				pageId: PAGE_ID
			}
		},
		...
		computed: {
			...
			filteredCities() {
				return this.shared.cities.filter(e => e.sub_pages.some(s => s.layout_id == LAYOUT_ID))
			}
		},
		methods: {
			setSelectedCity(city) {
				if(city) {
					const item = city.sub_pages.find(e => e.layout_id == LAYOUT_ID)
					if(item) {
						qstore.setSelectedCity(city)
						window.location.href = window.location.origin + item.path
					}
				} else {
					qstore.setSelectedCity(null)
				}
			},
			async fetchUser() {
				try {
					const url = new URL(location.origin + '/api/users/me')
			...
			async userPurchasesRequest() {
				console.log('userPurchasesRequest')
				try {
					const url = new URL(location.origin + '/api/users/me')
					const res = await fetch(url)
					if(!res.ok) throw new Error('Error')
					...
					if(this.user.user_purchased_objects.length !== result.user_purchased_objects.length) {
						this.isUserPurchasesUpdated = true
					}
					this.user = result
					
					if(!this.isUserPurchasesUpdated) {
						this.userPurchasesRequest()
					}
				} catch(err) {
					console.log(err)
					noti(false, 'Ошибка')
			...
			checkPurchasedItem(itemId) {
				if(!this.user) return false
				return this.user.user_purchased_objects.find(e => e.id == itemId)
			},
			async createPaymentModal(payment) {
				const result = await makePayment(payment)
				//console.log(result)
				if(result == 'success') {
					noti(true, 'Оплата произведена')
					//this.isShowPayBalanceForm = false
					this.userPurchasesRequest()
				} else if(result == 'fail') {
					noti(false, 'Ошибка')
				}
			},
			async testBuy(itemId) {
				this.isBuyObjectModalShowed = false
				try {
					if(!itemId) return;
					...
					if(!res.ok) throw new Error('Error')
					const result = await res.json()
					await this.createPaymentModal(result.payment)
					console.log(result)
			...
			isItemInRouteOrPlan(itemId) {
				console.log(this.shared.myRoute.planItems)
				const isItemInPlan = this.shared.myRoute.planItems.find(e => e.id == itemId)
				const isItemInRoute = this.shared.myRoute.routeItems.find(e => e.items.find(x => x.id == itemId))
				
				return isItemInPlan || isItemInRoute
			},
			addItemToMyRoutePlan(item) {
				//vux.addRoutePlanItem(item.id)
				qstore.addItemToMyRoutePlan(item)
			},
			addItemToMyRoutePlan2(item) {
				qstore.addItemToMyRoutePlan(item)
			},
				...
				*/
				...
				if(this.tags.length > 0) {
					...
					nested = nested.filter(e => {
						...
						let itemTags = e.field_tags ? e.field_tags.split(',').map(tag => tag.trim()) : []
			...
			async fetchData() {
				try {
					...
					const {data} = await apiService.pages.show(this.pageId, params)
					const items = generateFilters(data.sub_pages.map(e => {
						let gl = e.field_gallery ?? ''
		...
		async mounted() {
			try {
				...
				if(searchParams.get('bo')) {
					const buyObjectModalItem = this.items.find(e => e.id == searchParams.get('bo'))
					if(buyObjectModalItem) {
						this.buyObjectModalItem = buyObjectModalItem
						this.isBuyObjectModalShowed = true
					}
				}
			} catch(err) {
				console.log(err)
	...
	const makePayment = async (payment) => {
		try {
			...
			let checkoutResult = await new Promise((resolve, reject) => {
				checkout.on('success', () => {
					console.log(true, 'Оплата произведена')
					//noti(true, 'Оплата произведена')
					resolve('success')
					checkout.destroy();
				})
