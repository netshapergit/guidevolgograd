...
import { parseObject, objectTypes } from './utils.js'
import { routeCardDesc } from './c_route_card_desc.js'
document.addEventListener('DOMContentLoaded', function() {
	...
	new Vue({
		...
		methods: {
			setRoute() {
				qstore.setRoute([this.items])
			},
			getItemDescription(itemId) {
				return this.ttsData.find(e => e.id == itemId)
			},
			...
			appendRouteObjects(data) {
				...
				const ids = this.searchParams.ids.split(',').map(e => e.trim())
				
				const result = []
				//console.log(ids)
				ids.forEach(e => {
					const item = dataGal.find(d => d.id == e)
					if(item) {
						result.push(item)
					}
				})
		...
		mounted() {
			...
			let ttsData = parseObject(jQuery(this.$refs.ttsData).text(), [])
			this.$refs.ttsData.remove()
			console.log(ttsData)
			...
			ymaps.ready(async function() {
				...
				const data = await _this.fetchIds(_this.searchParams.ids.split(',').map(e => e.trim()).join())
				console.log(data)
				_this.appendRouteObjects(data.data)
			})
