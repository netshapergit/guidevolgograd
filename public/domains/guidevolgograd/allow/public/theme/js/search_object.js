	...
	new Vue({
		...
		computed: {
			isItemAlreadyInRouteOrPlan() {
				...
				return combinedItemsIds.includes(this.pageItem.id)
			},
			pageItemTicketsLink() {
		...
		methods: {
			removeItemFromMyRoutePlanByID() {
				qstore.removeItemFromMyRoutePlanByID(this.pageItem.id)
			},
			addToPlanById() {
				//if(!this.itemId) return;
				console.log('addToPlanById')
				const itemToAdd = window.SEARCH_ITEMS.find(e => e.item_data.id == this.pageItem.id)
				if(itemToAdd) {
